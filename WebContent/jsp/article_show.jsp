                    ...
                    <div class="header">
                        ...
                        <div class="author-img">
                            <a href="#" id="author-img-a" style="margin: 5px 0px 5px 5px;">
                                <img src="${requestScope.article.authorImg?requestScope.article.authorImg:requestScope.article.authorImg}"
<%--                                "http://localhost:8080/iDouBan/image/user_img/userPicture.jpg"}"--%>
                        ...
                        <div class="bottom_inner_div">
                            ...
                            <c:if test="${requestScope.starStatus == '已点赞文章' }">
                                <a href="/iDouBan/ArticleShowServlet?pre_method=article_star_cancel&method=article_show&article_id=${requestScope.article.articleId}"
                                   class="user_operation_btn" id="">${requestScope.article.starNum}取消点赞</a>
                            </c:if>
                            <c:if test="${requestScope.starStatus == '未点赞文章' }">
                                <a href="/iDouBan/ArticleShowServlet?pre_method=article_star&method=article_show&article_id=${requestScope.article.articleId}"
                                   class="user_operation_btn" id="">${requestScope.article.starNum}点赞</a>
                            </c:if>
                            <!-- 文章收藏 -->
                            <c:if test="${requestScope.collectionStatus == '已收藏文章' }">
                                <a href="/iDouBan/ArticleShowServlet?pre_method=article_collection_cancel&method=article_show&article_id=${requestScope.article.articleId}"
                                   class="user_operation_btn" id="">${requestScope.article.collectionNum}取消收藏</a>
                            </c:if>
                            <c:if test="${requestScope.collectionStatus == '未收藏文章' }">
                                <a href="/iDouBan/ArticleShowServlet?pre_method=article_collection&method=article_show&article_id=${requestScope.article.articleId}"
                                   class="user_operation_btn" id="">${requestScope.article.collectionNum}收藏</a>
                            </c:if>
                            <!-- 文章转发-->
                            <c:if test="${requestScope.shareStatus == '已转发文章' }">
                                <a href="/iDouBan/ArticleShowServlet?pre_method=article_share_cancel&method=article_show&article_id=${requestScope.article.articleId}"
                                   class="user_operation_btn" id="">${requestScope.article.shareNum}取消转发</a>
                            </c:if>
                            <c:if test="${requestScope.shareStatus == '未转发文章' }">
                                <a href="/iDouBan/ArticleShowServlet?pre_method=article_share&method=article_show&article_id=${requestScope.article.articleId}"
                                   class="user_operation_btn" id="">${requestScope.article.shareNum}转发</a>
                            </c:if>

                            <a href="javascript:void(0);" class="user_operation_btn"
                               id="">${requestScope.article.commentNum}回应</a>
        ...
        <div id="main_content-left-centre">
            ...
            <div class="comment-form-div">
                ...
                <form action="/iDouBan/ArticleShowServlet?pre_method=reply_content&method=article_show&article_id=${requestScope.article.articleId}"
                      id="replySubmit" name="reply_form" method="post">
                ...
                <form action="/iDouBan/ArticleShowServlet?pre_method=comment_content&method=article_show&article_id=${requestScope.article.articleId}"
                      id="onSubmit" name="comment_form" method="post">
                    <!-- 发表评论的文本框 -->
                    <div class="item">
                        <textarea name="comment-place" required id="comment-textarea" rows="4" cols="64" style="border-radius: 15px;background-color: rgba(248,248,248,0.63)"></textarea>
                        <br>
                    </div>
                    <%--隐藏表单--用来传输文本框的内容--content --%>
                ...
                <%--隐藏表单--提交 --%>
                <div class="submit-confirm" style="margin: 10px 20px;">
					          		<span>
					                	<button onclick="comment_sub()" id="submit_btn" style="width: 80px;height: 40px">评论</button>
					                </span>
                </div>
            </div>
            ...
            <div id="paging">
						 		<span id="page_number">
								当前第 <strong> <span id="currentPage_num"> 1</span> </strong> 页 总共 <strong> <span
                                        id="totalPage_num">1 </span> </strong> 页
								</span>
...
<script type="text/javascript">
    ...
    function toAjax(c) {
        ...
        xmlHttpRequest.send("method=comment_show&article_id=${requestScope.article.articleId}&currentPage=" + 1);
            ...
            for (let i = 0; i < obj.objects.length; i++) {
                ...
                if (obj.objects[i].starStatus === 1) {
                    $comment=
                        ...
                        '						        <p id=\"comment-msg\" style="margin: 10px 2px;height: auto;    line-height: 1.6;font-size: 13px;    width: 575px;word-wrap: break-word;overflow: hidden;">' + obj.objects[i].comMsg +
                        '							<a href="#submit-confirm" class=\"reply_a\" id=\"' + obj.objects[i].commentId + '\"  style=\"color : grey;float: right;margin: 9px 2px;\">回复</a>' +
                        //TODO：'						<a href=\"/iDouBan/ArticleShowServlet?pre_method=comment_star_cancel&method=article_show&comment_id=' + obj.objects[i].commentId + '&article_id='${requestScope.article.articleId} + '\" class="comment_star_a" id="127" style="color : grey;float: right;margin: 9px 2px;">取消点赞(' + obj.objects[i].comStar + ')</a>' +
                        '						</p>' +
                        '						    </div>' +
                        '<input type=\"hidden\"   value = \"' + obj.objects[i].commentId + '\">' +
                        '						</div>' +
                ...
                for (var j = 0; j < objReply.objects.length; j++) {
                    ...
                    if (objReply.objects[j].commentId === obj.objects[i].commentId) {
                        $reply =
                            ...
                            '						    <!-- 回复人的头像 -->' +
                            '						    <div class=\"pic\" style=\"margin: 13px 6px;width: 70px;float: left;\">' +
                            '						        	<img width=\"52\" height=\"52\" class="" src=\"' + objReply.objects[j].userReplyImg + '\" alt=\"用户头像\" style=\"margin: 2px 0;    border-radius: 4px;\">' +
                            '						    </div>' +
                            '						    <div class="content report-comment" style="float: left;height: auto;width: 590px;margin: 11px 0;">' +
                            '						        <div class="author">' +
                            '						        	<!-- 回复时间 -->' +
                            '						            <span class="">' + objReply.objects[j].replyTime + '</span>' +
                            '                           		<!--评论人的昵称-->' +
                            '						            <a href="#" class="" style="color:black;margin: 0 11px;">' + objReply.objects[j].userReplyFromNick + '</a>' +
                            '									 <span class="">回复</span>' +
                            '                                    <a href="#" class="" style="color:black;margin: 0 11px;">' + objReply.objects[j].userReplyToNick + '</a>' +
                            '						        </div>' +
                            '						' +
                            '						        <!-- 回复的内容-->' +
                            '						        <p id=\"comment-msg\" style="margin: 10px 0;height: auto;    line-height: 1.6;font-size: 13px;width: 530px;word-wrap: break-word;overflow: hidden;">' + objReply.objects[j].replyMsg +
