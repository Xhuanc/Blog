...
<div id="main_content">
    <div id="main_content-left">
        ...
        <div id="main_content-left-top">
            <div>
                <h2 style="margin-bottom: 20px;">所有人列表</h2>
            </div>
            ...
            <c:if test="${requestScope.p.totalPage!=0}">
                <div id="paging">
                    <a href="EveryoneListServlet?method=${requestScope.method}&currentPage=1" id="first_page"
                       class="page_contrller">首页</a>
                    <!-- 利用el表达式的三目运算符进行判断 -->
                    <a href="EveryoneListServlet?method=${requestScope.method}&currentPage=${(requestScope.p.currentPage==1)?1:requestScope.p.currentPage-1}"
                       id="pervious_page" class="page_contrller">上一页</a>
                    <a href="EveryoneListServlet?method=${requestScope.method}&currentPage=${(requestScope.p.currentPage==requestScope.p.totalPage)?requestScope.p.totalPage:requestScope.p.currentPage+1}"
                       id="next_page" class="page_contrller">下一页</a>
                    <a href="EveryoneListServlet?method=${requestScope.method}&currentPage=${requestScope.p.totalPage}"
                       id="last_page" class="page_contrller">尾页</a>
                    <span id="page_number" style="float: right"></span>
                </div>
            </c:if>
...
<script type="text/javascript">
    <%--document.getElementById("page_number").innerText=`当前第${requestScope.p.currentPage?requestScope.p.currentPage:0}页 总共${requestScope.p.totalPage?requestScope.p.totalPage:0}页`--%>
</script>
</body>
</html>